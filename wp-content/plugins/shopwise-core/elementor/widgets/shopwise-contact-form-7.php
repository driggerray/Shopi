...
class Shopwise_Contact_Form_7_Widget extends Widget_Base {
    use Shopwise_Helper;
    public function get_name() {
        return 'shopwise-contact-form-7';
    }
    ...
    protected function _register_controls() {
        $this->start_controls_section( 'shopwise_cf7_global_controls',
            [
                'label'=> esc_html__( 'Form Data', 'shopwise' ),
                'tab'=> Controls_Manager::TAB_CONTENT
            ]
        );
        $this->add_control('shopwise_cf7_form_id_control',
            [
                'label'=> esc_html__( 'Select Form', 'shopwise' ),
                'type'=> Controls_Manager::SELECT2,
                'multiple'=> false,
                'options'=> $this->shopwise_get_cf7(),
        ...
        $this->add_control( 'desc',
            [
                ...
                'pleaceholder' => esc_html__( 'Set a text.', 'shopwise' ),
            ]
        );
        ...
        $this->start_controls_section('cf7_form_style_section',
            [
                'label'=> esc_html__( 'Form Style', 'shopwise' ),
        ...
        $this->add_responsive_control( 'cf7_form_custom_width',
            [
                'label' => esc_html__( 'Form Max Width', 'shopwise' ),
        ...
        $this->add_responsive_control( 'cf7_form_input_height',
            [
                'label' => esc_html__( 'Input Height', 'shopwise' ),
                'type' => Controls_Manager::SLIDER,
                'range' => [
                    'px' => [
                        'min' => 0,
        ...
        // Style function
        $this->shopwise_style_controls($hide=array(),$id='cf7_form_normal_style',$selector='input:not(.wpcf7-submit),{{WRAPPER}} textarea');
        ...
        // Style function
        $this->shopwise_style_controls($hide=array('typo','margin'),$id='cf7_form_hover_style',$selector='input:not(.wpcf7-submit):hover,{{WRAPPER}} textarea:hover');
        ...
        $this->start_controls_tab( 'cf7_form_focus_tab',
            [ 'label'  => esc_html__( 'Focus', 'shopwise' ) ]
        );
        // Style function
        $this->shopwise_style_controls($hide=array('typo','margin'),$id='cf7_form_focus_style',$selector='input:not(.wpcf7-submit):focus,{{WRAPPER}} textarea:focus');
        ...
        $this->start_controls_section('cf7_formbtn_style_section',
            [
                'label' => esc_html__( 'Form Button Style', 'shopwise' ),
                'tab' => Controls_Manager::TAB_STYLE
            ]
        );
        ...
        // Style function
        $this->shopwise_style_controls($hide=array(),$id='cf7_formbtn_normal_style',$selector='.wpcf7-submit');
        ...
        // Style function
        $this->shopwise_style_controls($hide=array('typo','margin'),$id='cf7_formbtn_hover_style',$selector='.wpcf7-submit:hover');
    ...
    protected function render() {
        ...
        if (!empty($settings['shopwise_cf7_form_id_control'])){
			
			echo '<div class="klb-contact-form">';
			echo '<div class="heading_s1">';
			echo '<h2>'.esc_html($settings['title']).'</h2>';
			echo '</div>';
			echo '<p class="leads">'.esc_html($settings['desc']).'</p>';
			echo do_shortcode( '[contact-form-7 id="'.$settings['shopwise_cf7_form_id_control'].'"]' );
			echo '</div>';

        } else {
            esc_html_e('Please Select a Form','shopwise');
        }
    }
}
